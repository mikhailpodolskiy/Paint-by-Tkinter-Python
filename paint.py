...
class Paint():
	def __init__(self,root):
		self.root = root
		self.root.title("Приложение Paint")
		...
		self.color_frame = LabelFrame(self.root,text ="Цвет",font =('arial',15,'bold'),bd=5,relief=RIDGE,bg='white')
		...
		self.erase_button = Button(self.root,text="Ластик",bd=4,relief=RIDGE,width=8,command=self.eraser,bg="white")
		self.erase_button.place(x=0,y=187)

		self.clear_sreen_button =Button(self.root,text="Очистить",bd=4,relief=RIDGE,width=8,command=lambda : self.canvas.delete("all"),bg="white")
		self.clear_sreen_button.place(x=0,y=217)

		self.save_button =Button(self.root,text="Сохранить",bd=4,relief=RIDGE,width=8,command=self.save_paint,bg="white")
		self.save_button.place(x=0,y=247)

		self.canvas_color_button =Button(self.root,text="Фон",bd=4,relief=RIDGE,width=8,command=self.canvas_color,bg="white")
	...
	def save_paint(self):
		try:
			...
			messagebox.showinfo('Файл сохранен ','Изображение сохранено как'+str(filename))
